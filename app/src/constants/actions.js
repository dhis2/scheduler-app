export const JOBS_LOAD = 'JOBS_LOAD';
export const JOBS_LOAD_SUCCESS = 'JOBS_LOAD_SUCCESS';
export const JOBS_LOAD_ERROR = 'JOBS_LOAD_ERROR';

export const JOB_SAVE = 'JOB_SAVE';
export const JOB_SAVE_SUCCESS = 'JOB_SAVE_SUCCESS';
export const JOB_SAVE_ERROR = 'JOB_SAVE_ERROR';

export const JOB_POST = 'JOB_POST';
export const JOB_POST_SUCCESS = 'JOB_POST_SUCCESS';
export const JOB_POST_ERROR = 'JOB_POST_ERROR';

export const JOB_DELETE = 'JOB_DELETE';
export const JOB_DELETE_SUCCESS = 'JOB_DELETE_SUCCESS';
export const JOB_DELETE_ERROR = 'JOB_DELETE_ERROR';

export const JOB_RUN = 'JOB_RUN';
export const JOB_RUN_SUCCESS = 'JOB_RUN_SUCCESS';
export const JOB_RUN_ERROR = 'JOB_RUN_ERROR';

export const CONFIGURATION_LOAD = 'CONFIGURATION_LOAD';
export const CONFIGURATION_LOAD_SUCCESS = 'CONFIGURATION_LOAD_SUCCESS';
export const CONFIGURATION_LOAD_ERROR = 'CONFIGURATION_LOAD_ERROR';

export const ATTRIBUTE_OPTIONS_LOAD_SUCCESS = 'ATTRIBUTE_OPTIONS_LOAD_SUCCESS';
export const ATTRIBUTE_OPTIONS_LOAD_ERROR = 'ATTRIBUTE_OPTIONS_LOAD_ERROR';

export const JOB_EDIT = 'JOB_EDIT';
export const JOB_DISCARD = 'JOB_DISCARD';

export const NOT_AUTHORIZED = 'NOT_AUTHORIZED';
